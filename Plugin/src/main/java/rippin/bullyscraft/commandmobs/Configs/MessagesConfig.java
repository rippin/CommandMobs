...
import rippin.bullyscraft.commandmobs.Msgs;
...
public class MessagesConfig {
    ...
    private static void create(){
        if (!(getFile().exists())){
            plugin.getServer().getLogger().info("messages.yml not found. Creating now...");
            try {
                plugin.getDataFolder().mkdir();
                getFile().createNewFile();
                copy(plugin.getResource("messages.yml"), configFile);
                reload(); //reload yml just in case

                saveFile();
                plugin.getServer().getLogger().info("messages.yml has been created!");
            } catch (IOException e) {
