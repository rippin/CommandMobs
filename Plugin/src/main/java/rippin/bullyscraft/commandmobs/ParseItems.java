...
public class ParseItems {
    ...
    //Example LEATHER_HELMET Enchant:ENCHANT@LEVEL LEATHERCOLOR:COLOR
    ...
    public static String serializeItem(ItemStack item){
        ...
        if (item.getEnchantments().size() > 0){
           ...
           while (it.hasNext()){
               ...
               enchants = " " + "Enchant:" + entry.getKey().getName() + "@" + entry.getValue();
           }
        }
        if (item.getData() != null){

           data = "Data:" + Byte.toString(item.getData().getData());
        }
